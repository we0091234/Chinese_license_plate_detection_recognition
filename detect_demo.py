    ...
    for i, det in enumerate(pred):  # detections per image
        if len(det):
            ...
            for j in range(det.size()[0]):
                ...
                result_dict = get_plate_rec_landmark(orgimg, xyxy, conf,
                                                     landmarks, class_num,
                                                     device)
...
def draw_result(orgimg, dict_list):
    ...
    for result in dict_list:
        ...
        cv2.rectangle(orgimg, (rect_area[0], rect_area[1]),
                      (rect_area[2], rect_area[3]), clors[label], 2)  #画框
        cv2.putText(img, str(label), (rect_area[0], rect_area[1]),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, clors[label], 2)
